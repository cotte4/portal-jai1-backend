...
import { IrsCheckTrigger, FederalStatusNew } from '@prisma/client';
...
const mockService = () => ({
  ...
  runAllChecks: jest.fn().mockResolvedValue({ total: 10, succeeded: 9, failed: 1 }),
  runCheck: jest.fn().mockResolvedValue({ success: true, statusChanged: false, rawStatus: 'Return Received' }),
...
describe('IrsMonitorController', () => {
  let controller: IrsMonitorController;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: IrsMonitorService, useValue: service }],
    }).compile();

    controller = module.get<IrsMonitorController>(IrsMonitorController);
  });
  ...
  describe('runCheckAll()', () => {
    ...
    it('calls runAllChecks with manual trigger and admin id', async () => {
      ...
      // Wait for microtask queue (fire-and-forget)
  ...
  // ParseUUIDPipe is applied at the framework level (NestJS pipe execution),
  // so we test the pipe directly to verify it rejects invalid inputs.
